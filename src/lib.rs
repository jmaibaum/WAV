...
/// Reads in the given file and attempts to extract the audio data and header
/// from it.
/// 
/// # Errors
/// 
...
pub fn read_wav(reader: &mut dyn Read) -> std::io::Result<(Header, BitDepth)> {
    ...
    match wav.content {
                    ...
                    if c.id.as_str() == "data" {
                        if let riff::ChunkContent::Subchunk(v) = &subchunks[1].content {
                            match head.bits_per_sample {
                                ...
                                _ => return Err(std::io::Error::new(std::io::ErrorKind::Other, "Unsupported bit depth")),
                            };
                        }
                    }
                }
        ...
        _ => return Err(std::io::Error::new(std::io::ErrorKind::Other,"File not a WAVE file")),
    };

    if data == BitDepth::Empty {
        return Err(std::io::Error::new(std::io::ErrorKind::Other,"Could not parse audio data"));
    }
